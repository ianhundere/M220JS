...
(async () => {
	try {
		...
		const host = 'mongodb+srv://m220student:m220password@mflix-yi1fj.mongodb.net';
		...
		const projection = {};
		const cursor = await mflix.collection('movies').find(predicate, projection).toArray();
		...
		if (e instanceof MongoError && e.message.slice(0, 'Invalid Operation'.length) === 'Invalid Operation') {
